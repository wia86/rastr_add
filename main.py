...
# exe приложение:
#  pyinstaller  --noconsole main.py --name Помощник_2024_01_10 -i fav.ico --add-data "help;help" # --onefile
...
class CalcModel(GeneralSettings):
    ...
    def run_calc_task(self):
        ...
        log.debug(f'Запись параметров режима в excel.')
        full_breach = {}
        for key in self.breach:
        ...
        # Сводная.
        if len(full_breach):
            log.info(f'Формирование сводных таблиц ({self.book_path}).')
    ...
    def calc_file(self, rm):
        ...
        if self.task_calc['cb_control']:
            log.debug('Определение контролируемых элементов сети.')
        ...
        log.info(f"Сочетание {self.comb_id}: {self.info_srs['Наименование СРС']}")
        self.do_action(rm)
    ...
    def calc_comb(self, rm, comb: pd.DataFrame, source: str = 'combinatorics'):
        ...
        log.info(f"Сочетание {self.comb_id}: {all_name_srs}")
    ...
    def do_action(self, rm, comb=pd.DataFrame()):
        ...
        while True:
            ...
            if self.set_save["pa"] and self.pa.active(overloads):  # TODO overloads не задано
    ...
    def do_control(self, rm, comb=pd.DataFrame()):
        ...
        if not test_rgm:
            ...
            if tv.count:
                ...
                violation = True
                log.info(f'Выявлено {len(overloads_i)} превышений ДТН.')
            ...
            if tn.count:
                ...
                violation = True
                log.info(f'Выявлено {len(low_voltages)} точек недопустимого снижения напряжения.')
            ...
            if tn.count:
                ...
                violation = True
                log.info(f'Выявлено {len(high_voltage)} точек недопустимого превышения напряжения.')
...
class RastrModel:
    ...
    def cor(self,
        ...
        info = []
        if print_log:
            log.info(f"\t\tФункция cor: {keys=},  {values=}")
        keys = keys.replace(' ', '')
    ...
    def voltage_fine(self, choice: str = ''):
        ...
        """
        log.info('Проверка расчетного напряжения.')
        node = self.rastr.tables("node")
        ...
        if node.count:
            for name, ny, vras, umin in node.writesafearray('name,ny,vras,umin', "000"):
                log.warning(f"\tНапряжение ниже минимально-допустимого: {ny=}, {name=}, {vras=}, {umin=}")
    ...
    def table_add_row(self, table: str = '', tasks: str = '') -> int:
        ...
        log.info(f'\tВ таблицу <{table}> добавлена строка <{tasks}>, индекс <{index}>')
        return index
    ...
    def shn(self, choice: str = ''):
        ...
        """
        log.info("\tДобавлены зависимости СХН в таблицу узлы (uhom>100 nsx=1, uhom<100 nsx=2)")
        ...
        for i in range(10):  # максимальное число итераций
            ...
            if abs(ratio - 1) > eps:
                ...
                if not self.rgm('cor_pop'):
                    log.error(f"Аварийное завершение расчета, cor_pop: {zone=}, {new_pop=}")
                    return False
            else:
    ...
    def auto_shunt_rec(self, selection: str = '', only_auto_bsh: bool = False) -> dict:
        ...
        """
        log.debug(f'Поиск узлов с СКРМ {selection=}')
        all_auto_shunt = {}
        ...
        while i > -1:
            ...
            if AutoBsh:  # 105-126.5;ny=100
                if '(' in AutoBsh:
                    log.error(f'Ошибка в задании {AutoBsh=}')
                    i = node.FindNextSel(i)
                    continue
                if ';' in AutoBsh:
                    try:
            ...
            log.debug(f'Обнаружено СКРМ: {ny=} {name=} {ny_adjacency=} {ny_control=} {umin=} {umax=}')
            i = node.FindNextSel(i)
    ...
    def fill_field_index(self, name_tables: str):
        ...
        for name_table in name_tables.replace(' ', '').split(','):
            ...
            log.debug(f'В таблице {name_table} заполнено поле index.')
