...
def mainR ():#  головная процедура
    ...
    if GLR.vid_raschota == 1 OR (GLR.vid_raschota = 2 and GLR.PZ_word): if not objFSO.FolderExists(GLR.IzFolder): msgbox ( GLR.IzFolder + " - не найден"):GS.end_gl (): exit def
...
def rg2_raschot (): #  общие действия выполняемые с файлом 
    ...
    if GS.kod_rgm = 1: 
        GS.kod_rgm = rastr.rgm ("p")
    if GS.kod_rgm = 1: logging.info ( "\t" + "НОРМАЛЬНЫЙ РЕЖИМ НЕ МОДЕЛИРУЕТСЯ!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!, выход"): exit def
    ...
    if GLR.AutoShunt: 
        ...
        if GLR.Tabl_otlk_kontrol > 0 and GS.AutoShunt_list != "": GLR.XL_sheet.Cells(1,17).Value = GS.AutoShunt_list: GS.AutoShunt_list = ""

    if GLR.zad_temperatura  > 0: Tc_0_Sub ()#  обнулить расчетную температуру в ветвях районах и тд
    rastr.CalcIdop (RG.GradusZ,float(0) , ""): logging.info("\t" + "расчетная температура(rg2_raschot): " + str (RG.GradusZ) )       #    
    ...
    if GLR.MaxOtkl > UBound(RG.OTKL_masiv , 2) + 1:   
        ...
        logging.info( "!!!Количество элементов в сочетаниях уменьшено до количества отключаемых элементов равных "+ str(GLR.MaxOtkl)+"!!!" )  
    ...
    for i_comb = GLR.MinOtkl to GLR.MaxOtkl    #         цикл  ОТКЛЮЧЕНИЯ ПО ПОРЯДКУ  формирует RG.Otkl_Comb_tek из RG.OTKL_masiv         и запускает OTKL_Comb_tip
        if not (  GLR.not_n2 and i_comb=2): #  чтоб можно  было посчитать н-1 и н-3
            if i_comb = 3 and RG.temp_a_v_gost: logging.info( "\t"+ "n-3 по ГОСТу не считаем" ): exit for 
            if i_comb > 0:
                GLR.kol_otkl = i_comb
...
class rg2_tek_file:   #  RG  RG. для хранения параметров текущего расчетного файла
    ...
    def Kontrol_init ():
        ...
        logging.info ( "\t" + "контролитуемых ветвей + узлов:"+ str (UBound(KontrolVL , 1) + UBound(KontrolTrans , 1)+2)    + " + " +  str (UBound(KontrolNode , 1) +1) + " = " +  str (UBound(KontrolVL , 1) + UBound(KontrolTrans , 1)+2 + UBound(KontrolNode , 1) +1) )
    ...
    def OTKL1_masiv (): #     ОТКЛЮЧАЕМЫЕ ЭЛЕМЕНТЫ в массив
        ...
        logging.info ( "\t" + "отключаемых ветвей + узлов:"+ str (n_otkl_v) + " + " + str (n_otkl_n)+ " = " + str (n_otkl_v + n_otkl_n))
        if auto_est = True:
    ...
    public def fTest_Comb (): #  возвращает лож если не нужно считать режим и истина если нужно
        ...
        if ubound (arrNDX) = 1:  #  н-2
            if abs(delta_0) > GLR.viborka_comb or abs( delta_1 )> GLR.viborka_comb: 
                fTest_Comb = True
                if GLR.print_viborka: logging.info( "\t" + "н-2 сочетание проходит: " + dname0 + " (" + str(delta_1) + "%); "+ dname1 + " (" + str(delta_0) + "%)[" + str(comb.tip_comb) + "]" ) #  имя (его загрузка при отключении других элементов)
            else:
                if GLR.print_viborka: logging.info( "\t" + "н-2 сочетание отклонено:" + dname0 + " (" + str(delta_1) + "%); "+ dname1 + " (" + str(delta_0) + "%)[" + str(comb.tip_comb) + "]")
            ...
            if    (abs(delta_01 ) + abs( delta_02 )) > GLR.viborka_comb and (abs(delta_10 ) + abs( delta_12 )) > GLR.viborka_comb and (abs(delta_20 ) + abs( delta_21 )) > GLR.viborka_comb: 
                fTest_Comb = True
                if GLR.print_viborka: logging.info( "\t" + "н-3 сочетание проходит: " + dname0 + " (" + str(delta_01) +"% + "+ str(delta_02) + "%); "+ dname1 + " (" +  str(delta_10) +"% + "+ str(delta_12)  + "%)" + dname2 + " (" +  str(delta_20) +"% + "+ str(delta_21)  + "%)[" + str(tip_comb) + "]")#  имя (его загрузка при отключении других элементов)
            else:
                if GLR.print_viborka: logging.info( "\t" + "н-3 сочетание отклонено: "+ dname0 + " (" + str(delta_01) +"% + "+ str(delta_02) + "%); "+ dname1 + " (" +  str(delta_10) +"% + "+ str(delta_12)  + "%)" + dname2 + " (" +  str(delta_20) +"% + "+ str(delta_21)  + "%)[" + str(tip_comb) + "]")    # End def return
    ...
    def find_section  (): #  поиск сечения  
        ...
        for i = 0 to otkl_count - 1
            for ii = 0 to otkl_count - 1
                if abs (mOtkl_Otkl (i  , ii)) > GLR.viborka_comb: 
                    ...
                    N_section = N_section + 1        for i = 0 to otkl_count - 1
            N_pervoe = 0
            for ii = 0 to otkl_count - 1
        ...
        if tip_print = 1: 
            ...
            if GLR.print_viborka: 
               ...
               if GS.N_rg2_File = 0: GLR.XL_print_mKOO.ListObjects.Add (1,GLR.XL_print_mKOO.Range(GLR.XL_print_mKOO.UsedRange.address))#  таблица        if GLR.print_viborka:
            if tip_print = 1: 
                #  печать загаловки
    ...
    def print_max_tok ():
        ...
        for EACH vetv_kontrol in dict_Imax_kontrol.Items
            ...
            for EACH vetv_kontrol_sez in vetv_kontrol.dict_Imax_ikontrol.Items
                ...
                yo = yo + 1        if GLR.XL_max_tok.UsedRange.rows.count > 1:
            ...
            for each iComb in otkl_element
                ...
                if GLR.viborka_comb > 0: 
                    ...
                    if not mKO.fTest_Comb (): 
                        raschot_da = False 
                        GLR.kol_test_net = GLR.kol_test_net + 1                if raschot_da:
                    Comb.tip_comb = iComb
...
def PlaceOutages (): #  формирует otkl_add remont_add в RG.Otkl_Comb_tek Do Rgm
    ...
    for off_index = 0 to GLR.kol_otkl - 1 
        ...
        if RG.Otkl_Comb_tek(1 , off_index) = "vetv":                   #  ОТКЛЧЕНИЕ ВЕТВЬ
            ...
            if not rez: 
                logging.info ( "\t" + "PlaceOutages: Комбинация отклонена, тк ветвь уже была отключена: " + tablica.SelString( (RG.Otkl_Comb_tek(0 , off_index))) + " {" + tablica.cols.item("dname").Z(RG.Otkl_Comb_tek(0 , off_index)) + "}, N комб: " + str( GLR.N_rezh))
                exit def
    ...
    while ndx_node >= 0
        if tN11.cols.item("tranzit").Z(ndx_node) < 1:
            ...
            if tV10.count > 2: #  узловая похоже
                ...
                NNtranzit = NNtranzit + 1        ndx_node = tN11.FindNextSel(ndx_node)
    wend
            ...
            for po_x = 0 to ubound (masiv , 1)
                for po_y = 0 to ubound (masiv , 2)
                    if tab = "":
                        ...
                        if plus_txt != "": xl.Cells( y0 + po_y , x0 + po_x ).Value = str (rastr.Tables(tab).cols.item(par).Z( masiv (po_x , po_y) )) + plus_txt        elif raspolozhenie = "верт":
            for po_x = 0 to ubound (masiv , 2)
                for po_y = 0 to ubound (masiv , 1)
                    if tab = "":
                        ...
                        if plus_txt != "": xl.Cells( y0 + po_y , x0 + po_x ).Value = str (rastr.Tables(tab).cols.item(par).Z( masiv ( po_y ,  po_x) )) + plus_txt    elif nn = 1: #  если моссив
        if raspolozhenie = "гор":
...
def TablF_Sub ():  #  ШАПКА ТАБОИЦЫ
    ...
    GLR.XL_sheet.Cells(3,13).Value = "Допустимый длительный ток  "#  + "в " + RG.name_list (1) + "ний период, А"
...
def NodeTest ():  # востонавление питания узлов
    ...
    OffNodeCount = 0
    NodeOn = 0    ReDim TotalOnVetv(100)           #  tV15.Size глобальная переменная, записываем номера узлов
    ...
    def add (): #  
        ...
        if ndxV > -1:
            ...
            if OO = 0: 
                ...
                Iaz          = round (Ir / fIadtn_OO (ndxV) * 100 )        if ndxN > -1: 
    ...
    def print_end_p (zadanie_tip):  #  вывод dictVal  и общее форматирование(zadanie_tip 0 обычная работа, 1 анализ перегрузок без принта)
        ...
        if dictVal.count> 0:
                        ...
                        for i = max_count to 2 step -1 #  от максимального количества отключений до 2
                            ...
                            for i_Key = 0 to dict_otkl_count.count-1 #  находим ключ или имя
                                if i = dict_otkl_count.Items()(i_Key): #  нашли 
                                        ...
                                        if val1(cKolOtkl) > 1 and val1(cTip) = "-1" and val1(cK) = i_Kont:
                                            if val1(cR1) = name_tekk and val1(cR1) != "" and val1(cR1) != "-"and val1(cO) != "" and val1(cO) != "-": 
                                                if not dict_Otkl_name.exists (val1(cO)): #  можно и нада менять сК1 и cO местами
                                                    ...
                                                    val1(cKeyO) = temp9
                                                    dictVal.Item(i_key2) = val1                                            if val1(cKolOtkl) > 2:
                                    ...
                                    if not dict_Otkl_name.exists (name_tekk): dict_Otkl_name.add (name_tekk,1)                         for i = max_count to 2 step -1 #  от максимального количества отключений до 2
                            for i_Key = 0 to dict_otkl_count.count-1 #  находим ключ или имя
                                if i = dict_otkl_count.Items()(i_Key): #  нашли 
            ...
            if GLR.PZ_word: 
                ...
                cursorRis.Font.Name = "Times  Roman"                redim arr_Kontr_el_n (3) #  # dim
                            ...
                            if iiii> 0 and iii = dictVal.item(iiii) (cKolOtkl):
                                ...
                                if Kontr_el_n.TipKontr  = "vetv":
                                    ...
                                    if float(dictVal.item(iiii) (cUr)) < Kontr_el_n.U_min_r or Kontr_el_n.U_min_r =0: 
                                        ...
                                        Kontr_el_n.U_min_r = float(dictVal.item(iiii) (cUr))                                if otklonenie_max_test:
                                    ...
                                    for  nnn = 0 to dict_OR_tek.count-1
                                        ...
                                        if iii > 1:
                                            ...
                                            if dict_temp.count > 1:
                                                ...
                                                cursor.TypeText (  " в схеме ремонта "  + dict_temp.keys()(0) )                                        if not nnn = dict_OR_tek.count-1: cursor.TypeText ( ";" + "\n"  ) else: cursor.TypeText ( "." + "\n"  )
            ...
            for i = LBound(RG.KontrolVL) to UBound(RG.KontrolVL)                                                  #  цикл по КОНТРОЛ ВЛ
                ...
                if max_zag_ddtn > 100: #  есть перегрузка ддтн
                    if not ( RG.temp_a_v_gost and GLR.kol_otkl = 2 and max_zag_adtn < 100): #  учет по гост в н-2 адтн
                        ...
                        if max_zag_ddtn > GLR.zagruz_add_tab: ADD_REZHIM_tab = 1                if max_zag_ddtn > 100 and GLR.picture_add: name_zagruzka_ris = f_name_zagruzka_ris (name_zagruzka_ris , tV21.cols.item("dname").ZS(ndxTab) , max_zag_ddtn , tV21.cols.item("i_max").ZS(ndx_gr_Imax) ,  tV21.cols.item("i_dop_r").ZS(ndxTab), fIadtn (ndxTab),"")
            ...
            for i = LBound(RG.KontrolTrans) to UBound(RG.KontrolTrans)                                            #   цикл по КОНТРОЛ ТРАНС
                ...
                if max_zag_ddtn > 100: #  последовательная обмотка АТ  или тр
                    if not ( RG.temp_a_v_gost and GLR.kol_otkl = 2 and max_zag_adtn < 100): #  учет по гост в н-2 адтн
                        ...
                        if GLR.protokol_XL: GLR.overload.Idz = max_zag_ddtn: GLR.overload.add  ()
                        if max_zag_ddtn > GLR.zagruz_add_tab: ADD_REZHIM_tab = 1                if max_zag_ddtn > 100 and GLR.picture_add: name_zagruzka_ris = f_name_zagruzka_ris (name_zagruzka_ris , tV21.cols.item("dname").ZS(ndxTab) , max_zag_ddtn , tV21.cols.item("i_max").ZS(ndxTab) ,  tV21.cols.item("i_dop_r").ZS(ndxTab), fIadtn (ndxTab) , "")
                if max_zagruzka_ndxTab_OO > 100: #  общая обмотка АТ
                    if not ( RG.temp_a_v_gost and GLR.kol_otkl = 2 and max_zag_adtn < 100): #  учет по гост в н-2 адтн
                        ...
                        if max_zagruzka_ndxTab_OO > GLR.zagruz_add_tab: ADD_REZHIM_tab = 1                if max_zagruzka_ndxTab_OO > 100 and GLR.picture_add: name_zagruzka_ris = f_name_zagruzka_ris (name_zagruzka_ris , tV21.cols.item("dname").ZS(ndxTab) , max_zagruzka_ndxTab_OO , I_OO , tV21.cols.item("IdopOO").Z(ndxTab), fIadtn_OO (ndxTab), " OO ")
    ...
    while ndx >= 0 #  
        if tVT42.count > 0: 
            if tVT42.cols.item("groupid").Z(ndx) > 0: 
                ...
                tV22.cols.item("Kontrol").calc (1)        ndx = tVT42.FindNextSel( ndx )
    wend
...
def add_tabl_KontrOtkl(): 
    ...
    GLR.XL_sheet.Cells(GLR.Y_list + GLR.Y_VL_Trans_V + 5, GLR.X_list).Value = GS.AutoShunt_list
    ...
    if GS.kod_rgm =1: #  записывает расчетные значения в таблицу
        ...
        if GLR.Tabl_otlk_kontrol = 1:
            ...
            for i = LBound(RG.KontrolTrans) To UBound(RG.KontrolTrans) #  цикл записи расчетных значений-ветви Trans_Trans_Trans_Trans_Trans_Trans_Trans_Trans_
                ...
                GLR.XL_sheet.Cells(GLR.Y_list + GLR.Y_VL + i + 1 + n + 1, GLR.X_list).Value = tV23.cols.item("Imax").Z(ndx)   # запись ток                if tV23.cols.item("KontrOO").Z(ndx) = True:   #  истина если контроль ОО АТ
    ...
    if tip_zad_ris = 4: #  4 - замена рисунков в ворд из rg2
        ...
        for Each objFile in objFSO.GetFolder(GLR.Folder_rg2_ris).Files#  цикл по файлам в  указанной папке
            ...
            if not  RG.Name_st = "не подходит" and objFile.type = "Файл режима rg2": 
                ...
                rastr.CalcIdop (RG.GradusZ,float(0) , "" ): logging.info("\t" & "расчетная температура(mainR): " & str (RG.GradusZ) )
...
def RisunokPrint ():  #  вставить рисунок в ворд/ сохранить rg2
    ...
    if GLR.risunok_rg2: 
        ...
        logging.info( "сохранен файл: " & Left( GLR.Folder_rg2 & "\"  & f_txt_name_good ( RG.Name_Base    & " (" & GLR.name_ris1 & str (GLR.number_pict) & ")"& RGR.name_ris(5)),250) & ".rg2")
                                    ...
                                    if fUslovieZad (vibor_PA & "&uslovie&Nstep=" & str(i) ): #  функция проверяет выполнение условия (выборка: zadanie sta Nstep )
                                        if tAZ.cols.item("action").Z(ndx_az) < 2: #   0 откл|1 вкл
                                            if tAZ.cols.item("tabl").Z(ndx_az) = 0: #    0  узел
                                                ...
                                                if ndx_n >-1: DName_sub ("node",ndx_n) else: logging.info ( "не найден узел (automatika,tip=" & str (tip) & "), задание:" & str (zadanie) )
                                                if tAZ.cols.item("action").Z(ndx_az)  = 0: #  отключить
                                                    ...
                                                    if fNodeSta ("ndx" ,ndx_n, 1 ): # "ndx"/"ny"; 121 ; vkl_otkl= 1 отключить/ 0 включить)
                                                        ...
                                                        logging.info ( "\t" & "узел отключен до (automatika,tip=" & str (tip) & "), " &  rastr.Tables("node").SelString(ndx_n) & tN26.cols.item("dname").ZS(ndx_n) & ", N комб: " & str (GLR.N_rezh))
                                                    ...
                                                    if fNodeSta ("ndx" , ndx_n , 0): # "ndx"/"ny"; 121 ; vkl_otkl= 1 отключить/ 0 включить)
                                                        ...
                                                        logging.info ( "\t" & "ветвь отключена до (automatika,tip=" & str (tip) & "), " & rastr.Tables("node").SelString(ndx_n) & " {" & tN26.cols.item("dname").ZS(ndx_n) & "}, N комб: " & str (GLR.N_rezh))                                            elif tAZ.cols.item("tabl").Z(ndx_az) = 1: #    1 ветвь

                                                if instr (tAZ.cols.item("kluch").Z(ndx_az) , ",") > 1:  #  задано ip,iq,np
                                                    ndx_v = fNDX ("vetv" , tAZ.cols.item("kluch").Z(ndx_az) )
                                                    if  ndx_v = -1:  
                                                        logging.info ( "не найдена ветвь (automatika,tip=" & str (tip) & "), задание:" &  str (zadanie) & "не найден v0/1 ndx_v " & str( ndx_v ))
                                                    else:
                                                        ...
                                                        if tAZ.cols.item("action").Z(ndx_az)  = 0: #  отключить
                                                            ...
                                                            if fVetv_Sta ("ndx" , ndx_v , 1):     #   "ndx"/"groupid"/"kluch"; "ip=1,iq=2,np=0"; vkl_otkl= 1 отключить/ 0 включить)
                                                                ...
                                                                logging.info ( "\t" & "ветвь отключена до (automatika,tip=" & str (tip) & "), " &  rastr.Tables("vetv").SelString(ndx_v) & " {" & tV26.cols.item("dname").ZS(ndx_v) & "}, N комб: " & str (GLR.N_rezh))
                                                    ...
                                                    if ndx_v < 0:  
                                                        logging.info ( "\t" & "не найден v0 groupid " & str( tAZ.cols.item("kluch").Z(ndx_az) )) #  если строка, те не найтен NDX, те ошибка в задании
                                                    else:
                                                        if tAZ.cols.item("action").Z(ndx_az)  = 0: #  отключить
                                                            ...
                                                            if fVetv_Sta ("groupid" , tAZ.cols.item("kluch").Z(ndx_az) , 1):     # "ndx"/"groupid"/"kluch"; "ip=1,iq=2,np=0"; vkl_otkl= 1 отключить/ 0 включить)
                                                                ...
                                                                logging.info ( "\t" & "ветвь отключена до (automatika,tip=" & str (tip) & "), " &  rastr.Tables("vetv").SelString(ndx_v)& ", N комб: " & str (GLR.N_rezh))
                                        ...
                                        if tAZ.cols.item("action").Z(ndx_az) = 2 or tAZ.cols.item("action").Z(ndx_az) = 3: #   2 ОН|3 ОГ
                                            ...
                                            if tAZ.cols.item("value").ZS(ndx_az) = "": logging.info ( "\t" & "значение не указано, N=" + tAZ.cols.item("N").ZS(ndx_az) + ",Nstep=" + tAZ.cols.item("Nstep").ZS(ndx_az)):  exit def
                                            value_m = split(tAZ.cols.item("value").ZS(ndx_az) , "*")
                                            if isnumeric (value_m(0)): value_z = float(value_m(0)) else: logging.info ( "\t" & "значение не является числом, N=" + tAZ.cols.item("N").ZS(ndx_az) + ",Nstep=" + tAZ.cols.item("Nstep").ZS(ndx_az)):  exit def

                                            if tAZ.cols.item("action").Z(ndx_az) = 2 and tAZ.cols.item("tabl").Z(ndx_az) = 0:       #  ограничение НАГРУЗКИ УЗЛА                             
                                                RG.loadRGM = True
                                                ndx_n = fNDX("node" , tAZ.cols.item("kluch").ZS(ndx_az) )
                                                if ndx_n = -1:  
                                                    logging.info ( "не найден узел (automatika,tip=" & str (tip) & "), задание:" & str (zadanie)& ", узел номер:" & tAZ.cols.item("kluch").ZS(ndx_az)  )
                                                else:                
                                                    ...
                                                    if pn < value_z: 
                                                        ...
                                                        pn = tN26.cols.item("pn").Z(ndx_n) 
                                                        if not TXTrez (i) = "": TXTrez (i) = TXTrez (i) + ", "
                                                        TXTrez (i) = TXTrez (i) +   "ограничение нагрузки на " & str(round(pn0 - pn ,0)) & " МВт"  # & "  на "& tN26.cols.item("dname").ZS(ndx_n)                                             elif tAZ.cols.item("action").Z(ndx_az) = 2 and tAZ.cols.item("tabl").Z(ndx_az) = 4:   #  ограничение нагрузки НАГРУЗОЧНОЙ ГРУППЫ
                                                RG.loadRGM = True
                                                ndx_ng = fNDX("ngroup" , tAZ.cols.item("kluch").ZS(ndx_az) )
                                                if ndx_ng = -1:  
                                                    logging.info ( "не найден номер нагр. группы (automatika,tip=" & str (tip) & "), задание:" &  str (zadanie) & ",  номер:" & tAZ.cols.item("kluch").ZS(ndx_az))
                                                else:                  
                                                    ...
                                                    if png < value_z: 
                                                        ...
                                                        if not TXTrez (i) = "": TXTrez (i) = TXTrez (i) + ", "
                                                        TXTrez (i) = TXTrez (i) +   "ограничение нагрузки nga= " &  tAZ.cols.item("kluch").ZS(ndx_az) & " на " &  str ( round(png0 - png ,0)) & " МВт"                                            elif tAZ.cols.item("action").Z(ndx_az) = 2 and tAZ.cols.item("tabl").Z(ndx_az) = 2:   #  ограничение НАГРУЗКИ РАЙОНА
                                                RG.loadRGM = True
                                                ndx_nr = fNDX("area" , tAZ.cols.item("kluch").ZS(ndx_az) )
                                                
                                                if ndx_nr = -1:  
                                                    logging.info ( "не найден номер района (automatika,tip=" & str (tip) & "), задание:" & str (zadanie) & ",  номер:"  & tAZ.cols.item("kluch").ZS(ndx_az))
                                                else:                 
                                                    ...
                                                    if pnr < value_z: 
                                                        ...
                                                        if not TXTrez (i) = "": TXTrez (i) = TXTrez (i) + ", "
                                                        TXTrez (i) = TXTrez (i) +    "ограничение нагрузки " & str  (round(pnr0 - pnr ,0)) & " МВт"                                            elif tAZ.cols.item("action").Z(ndx_az) = 2 and tAZ.cols.item("tabl").Z(ndx_az) = 3:   #  ограничение НАГРУЗКИ территории
                                                RG.loadRGM = True
                                                ndx_nr = fNDX("area2" , tAZ.cols.item("kluch").ZS(ndx_az) )
                                                
                                                if ndx_nr = -1:  
                                                    logging.info ( "не найден номер территории (automatika,tip=" & str (tip) & "), задание:" & str (zadanie) & ",  номер:"  & tAZ.cols.item("kluch").ZS(ndx_az))
                                                else:                 
                                                    ...
                                                    if pnr < value_z: 
                                                        ...
                                                        if not TXTrez (i) = "": TXTrez (i) = TXTrez (i) + ", "
                                                        TXTrez (i) = TXTrez (i) +    "ограничение нагрузки " & str  (round(pnr0 - pnr ,0)) & " МВт"                                            elif tAZ.cols.item("action").Z(ndx_az) = 3 and tAZ.cols.item("tabl").Z(ndx_az) = 0:   #  ограничение ГЕНЕРАЦИИ УЗЛА
                                                RG.loadRGM = True
                                                ndx_g = fNDX("node" ,tAZ.cols.item("kluch").ZS(ndx_az))
                                                
                                                if ndx_g = -1:  
                                                    logging.info ( "не найден номер узла (automatika,tip=" & str (tip) & "), задание:" & str (zadanie) & ",  номер:"  & tAZ.cols.item("kluch").ZS(ndx_az))
                                                else:              
                                                    ...
                                                    if pg < value_z: 
                                                        ...
                                                        pg = tN26.cols.item("pg").Z(ndx_g) 
                                                        if not TXTrez (i) = "": TXTrez (i) = TXTrez (i) + ", "
                                                        TXTrez (i) = TXTrez (i) +   "ограничение генерации на " & str ( round(pg0 - pg ,0)) & " МВт"  # & "  на "& tN26.cols.item("dname").ZS(ndx_n)                                             elif tAZ.cols.item("action").Z(ndx_az) = 3 and tAZ.cols.item("tabl").Z(ndx_az) = 5:   #  ограничение ГЕНЕРАЦИИ ГЕНЕРАТОРА

                                                RG.loadRGM = True
                                                ndx_gg = fNDX("Generator" , tAZ.cols.item("kluch").ZS(ndx_az) )
                                                
                                                if ndx_gg = -1:  
                                                    logging.info ( "не найден номер генератора (automatika,tip=" & str (tip) & "), задание:" & str (zadanie) & ",  номер:"  & tAZ.cols.item("kluch").ZS(ndx_az))
                                                else:             
                                                    ...
                                                    if value_z > 0: # нада снизить  ген
                                                        ...
                                                        if tGen.cols.item("sta").Z(ndx_gg): #  истина откл ген
                                                            ...
                                                            pgg = tGen.cols.item("P").Z(ndx_gg) 
                                                            if not TXTrez (i) = "": TXTrez (i) = TXTrez (i) + ", "
                                                            TXTrez (i) = TXTrez (i) +   "увеличение генерации на " & str (round(-pgg0 + pgg ,0)) & " МВт"  # & "  на "& tN26.cols.item("dname").ZS(ndx_n)                                         if tAZ.cols.item("action").Z(ndx_az) = 4:                                                #  вызов процедуры cor
                                            RG.loadRGM = True
                                        ...
                                        if tip = 2: # (если otkl_add или remont_add то не нада расчитывать )
                                            ...
                                            if KontrolEl (0) = "node": 
                                                ...
                                                if tAZ.cols.item("setpoint").ZS(ndx_az): 
                                                    ...
                                                    if tV26.cols.item("i_zag").ZN(KontrolEl (1)) < 100: exit for                                            if instr (tAZ.cols.item("value").ZS(ndx_az),"*") > 0  and exit_cycle = 0: cycle_while = 1                                    if cycle_while = 0: ndx_az = tAZ.findnextsel (ndx_az)
                                wend
...
def fParam_kluch (tabl , param , kluch):  #  функция 
    ...
    tTabl.setsel (kluch)
    index = tTabl.FindNextSel(-1)
    if index = -1:         logging.info ( "fParam_kluch в таблице " & tabl & " не найдено " & param & " ключ " & str (kluch  )  )
    fParam_kkluch = tTabl.cols.item(param).Z(index)
# End def return
# *******************РАСЧЕТ РЕЖИМОВ****************************************************************************************************************************
...
def f_name_zagruzka_ris (name_zagruzka_ris , name , zagruzka , I_maxx , Ipr , I_adtn , dop_name): 
    ...
    if I_adtn  > Ipr * 1.01:
        ...
        if name_zagruzka_ris = "": #  имя Загрузка для рисунков
            ...
            f_name_zagruzka_ris = name_zagruzka_ris & ", " & name & dop_name &          " - " & str (Round(zagruzka,0))  & " % (" & str (I_maxx) & " А) от Iддтн = Iадтн = "& str (round (Ipr)) & " А"   # имя контр# End def return
# *******************РАСЧЕТ РЕЖИМОВ****************************************************************************************************************************
          ...
          if nyk > 0:  vrs = TabNode.cols.item("vras").ZN(nyk_ndx) else: vrs = TabNode.cols.item("vras").ZN(ndx)         
            ...
            if (not ish_sta = TabNode.cols.item("sta").Z(ndx)): #  изм состояния узла
                if TabNode.cols.item("sta").Z(ndx): 
                    ...
                    list_info = list_info +  ", узел включен"            list_info = list_info +  " (Uрез=" & str(round( vrs)) & "). "#  & vbCrLf
...
def RGM_kor (txt):
    ...
    if GS.kod_rgm = 1:
        ...
        logging.info ("\t" + "расчет режима:" + txt + " !!! РАЗВАЛИЛСЯ!!!")
    else:
        logging.info ("\t" + "расчет режима: " + txt)
            ...
            while ndx_g >= 0 #  
                if tG.cols.item("NumPQ").ZN(ndx_g) = 0:
                    if tG.cols.item("Qmax").ZN(ndx_g) = 0 and qmax != 0: 
                        tG.cols.item("Qmax").ZN(ndx_g) = qmax * tG.cols.item("P").ZN(ndx_g) / pg #: logging.info ("узел " & tN33.cols.item("ny").ZN(ndx)  & " ген " & tG.cols.item("Num").ZN(ndx_g)   &  " qmax " &  tG.cols.item("Qmax").ZN(ndx_g))

                    if tG.cols.item("Qmin").ZN(ndx_g) = 0 and qmin != 0: 
                        tG.cols.item("Qmin").ZN(ndx_g) = qmin * tG.cols.item("P").ZN(ndx_g) / pg #: logging.info ("узел " & tN33.cols.item("ny").ZN(ndx)  & " ген " & tG.cols.item("Num").ZN(ndx_g)   &  " qmin " &  tG.cols.item("Qmin").ZN(ndx_g))                ndx_g = tG.FindNextSel(ndx_g)
            wend 
...
def  name_txt_korr (): #  удалить пробелы в начале и конце, заменить два пробела на один, английские менять на русские буквы
    logging.info( "\t" & " удалить пробелы в начале и конце, заменить два пробела на один, английские менять на русские буквы" )
...
def uhom_korr_sub (set_sel):  #  исправить номинальные напряжения в узлах (выборка в таблице узлы)
    ...
    while j != -1    #  цикл по узлам
        ...
        if kk = False: logging.info  ("\t" &"\t" & " Ошибка в узле " & tN35.cols.item("ny").ZS(j) &  "("  &  tN35.cols.item("name").ZS(j)  & ") U= " & cuhom.ZS(j) & " - не распознан клас напряжения")
...
def kontrol_rg2_sub (zad):   #  контроль  zad = array (наряжений 0, токов 1, генераторы 2 , сечений 3 , район 4  , территория 5 , объединение 6, выботка в таблице узлы 7)
    ...
    if zad (0): 
        ...
        for i = 0 to ubound (uh)
            ...
            while j != -1    #  
                if tN36.cols.item("vras").Z(j) > unr (i) or tN36.cols.item("vras").Z(j) < umin (i): 
                    logging.info ("\t" &"\t" & "ВНИМАНИЕ НАПРЯЖЕНИЕ! ny=" & tN36.cols.item("ny").ZS(j) & ", имя: "& tN36.cols.item("name").ZS(j) &",vras = "& tN36.cols.item("vras").ZS(j) &",uhom = "& tN36.cols.item("uhom").ZS(j) )
        ...
        if tN36.count > 0: 
            j = tN36.FindNextSel(-1)
            while j != -1    
                logging.info ("\t" & "\t" &  "ВНИМАНИЕ НАПРЯЖЕНИЕ(otv_min)! ny=" & tN36.cols.item("ny").ZS(j) & ", имя: "& tN36.cols.item("name").ZS(j) & ",vras = "& tN36.cols.item("vras").ZS(j) &", otv_min="& tN36.cols.item("otv_min").ZS(j) )
                j = tN36.FindNextSel(j)
            wend    # #  ТОКИ
    if zad (1): 
        rastr.CalcIdop (RG.GradusZ , float(0) , ""  )
        logging.info("\t" &  "контроль токовой загрузки, расчетная температура: " & str (RG.GradusZ) ) 
        ...
        if tV32.count > 0: # есть превышений
            ...
            while j > -1    
                logging.info ("\t" &"\t" &  "ВНИМАНИЕ ТОКИ! vetv:" & tV32.SelString(j) & ", " & tV32.cols.item("name").ZS(j) & " - " & tV32.cols.item("i_zag").ZS(j) )
        ...
        if tV32.count > 0: # 
            ...
            while j > -1  
                if tV32.cols.item("n_it").Z(j)>0:
                    ...
                    if gr_I_T.count = 0:  logging.info ("\t" &"\t" &  "ВНИМАНИЕ graphikIT! vetv:" & tV32.SelString(j) & ", " & tV32.cols.item("name").ZS(j) & ", n_it=" & tV32.cols.item("n_it").ZS(j) & " не найдена в таблице График_Iдоп_от_Т(graphikIT)" )

                if tV32.cols.item("n_it_av").Z(j)>0:
                    gr_I_T.setsel ("Num=" & tV32.cols.item("n_it_av").ZS(j) )
                    if gr_I_T.count = 0:  logging.info ("\t" &"\t" &  "ВНИМАНИЕ graphikIT! vetv:" & tV32.SelString(j) & ", " & tV32.cols.item("name").ZS(j) & ", n_it_av=" & tV32.cols.item("n_it_av").ZS(j) & " не найдена в таблице График_Iдоп_от_Т(graphikIT)" )
    ...
    if zad (2):   
        ...
        j = tG.FindNextSel(-1)
        while j != -1    # 
            if tG.cols.item("Pmin").Z(j) > 0 and tG.cols.item("P").Z(j) < tG.cols.item("Pmin").Z(j):  logging.info ("\t" &"\t" &  "ВНИМАНИЕ! ГЕНЕРАТОР: " & tG.cols.item("Name").ZS(j) & " (Num="& tG.cols.item("Num").ZS(j) & ",ny=" & tG.cols.item("Node").ZS(j) &"), P="& tG.cols.item("P").ZS(j) & ", Pmin=" & tG.cols.item("Pmin").ZS(j))
            if tG.cols.item("Pmax").Z(j) > 0 and tG.cols.item("P").Z(j) > tG.cols.item("Pmax").Z(j):  logging.info ("\t" &"\t" &  "ВНИМАНИЕ! ГЕНЕРАТОР: " & tG.cols.item("Name").ZS(j) & " (Num="& tG.cols.item("Num").ZS(j) & ",ny=" & tG.cols.item("Node").ZS(j) &"), P="& tG.cols.item("P").ZS(j) & ", Pmax=" & tG.cols.item("Pmax").ZS(j))
            if tG.cols.item("NumPQ").Z(j)> 0: 
                PQZ.setsel ("Num=" & tG.cols.item("NumPQ").ZS(j) )
                if PQZ.count = 0: logging.info ("\t" &"\t" &  "ВНИМАНИЕ! ГЕНЕРАТОР: " & tG.cols.item("Name").ZS(j) & " (Num="& tG.cols.item("Num").ZS(j) & ",ny=" & tG.cols.item("Node").ZS(j) &"), NumPQ="& tG.cols.item("NumPQ").ZS(j) & " не найден в таблице PQ-диаграммы (graphik2)" )
    ...
    if zad (3): 
        if tS.size = 0: 
            logging.info ("\t" & "файл сечений не загружеин или сечения отсутствуют"  )
        else:
            ...
            if tS.cols.item("pmax").Z(j) > 0:
                while j != -1    # 
                    if tS.cols.item("psech").Z(j) > tS.cols.item("pmax").Z(j)+ 0.01:  
                        logging.info ("\t" & "\t" &  "ВНИМАНИЕ! сечение: " & tS.cols.item("name").ZS(j) & " ("& tS.cols.item("ns").ZS(j) & "), P: "& tS.cols.item("psech").ZS(j) & ", Pmax: " & tS.cols.item("pmax").ZS(j)& ", отклонение: " & str(round(tS.cols.item("pmax").Z(j)-tS.cols.item("psech").Z(j))))

                    j = tS.FindNextSel(j)
                wend    
    #  Районы
    if zad (4): 
        logging.info ("\t" & "контроль pop_zad районов"  )
        tA1.setsel("pop_zad>0") 
        j = tA1.FindNextSel(-1)
        while j != -1    # 
            procent = abs (tA1.cols.item("pop_zad").Z(j) - tA1.cols.item("pop").Z(j))/tA1.cols.item("pop_zad").Z(j)
            if procent > 0.01:  
                logging.info ("\t" &"\t" &  "ВНИМАНИЕ! район: " & tA1.cols.item("name").ZS(j) & " ("& tA1.cols.item("na").ZS(j) & "), pop: "& tA1.cols.item("pop").ZS(j) & ", pop_zad: " & tA1.cols.item("pop_zad").ZS(j)& ", отклонение: " & str(round(tA1.cols.item("pop_zad").Z(j)-tA1.cols.item("pop").Z(j))) & " или " & str(round(procent*100)) & " %")
    ...
    #  территории
    if zad (5): 
        logging.info ("\t" & "контроль pop_zad территорий"  )
        tA2.setsel("pop_zad>0") 
        j = tA2.FindNextSel(-1)
        while j != -1    # 
            procent = abs (tA2.cols.item("pop_zad").Z(j) - tA2.cols.item("pop").Z(j)) / tA2.cols.item("pop_zad").Z(j)
            if  procent > 0.01:  
                logging.info ("\t" &"\t" &  "ВНИМАНИЕ! территория: " & tA2.cols.item("name").ZS(j) & " ("& tA2.cols.item("npa").ZS(j) & "), pop: "& tA2.cols.item("pop").ZS(j) & ", pop_zad: " & tA2.cols.item("pop_zad").ZS(j)& ", отклонение: " & str(round(tA2.cols.item("pop_zad").Z(j)-tA2.cols.item("pop").Z(j))) & " или " & str(round(procent*100)) & " %")
    ...
    #  объединение     
    if zad (6): 
        logging.info ("\t" & "контроль pop_zad объединений"  )
        tDA.setsel("pop_zad>0") 
        j = tDA.FindNextSel(-1)
        while j != -1    # 
            procent = abs (tDA.cols.item("pop_zad").Z(j) - tDA.cols.item("pp").Z(j)) / tDA.cols.item("pop_zad").Z(j)
            if  procent > 0.01:
                logging.info ("\t" &"\t" &  "ВНИМАНИЕ!объединение: " & tDA.cols.item("name").ZS(j) & " (" & tDA.cols.item("no").ZS(j) & "), pop: "& tDA.cols.item("pp").ZS(j) & ", pop_zad: " & tDA.cols.item("pop_zad").ZS(j)& ", отклонение: " & str(round(tDA.cols.item("pop_zad").Z(j)-tDA.cols.item("pp").Z(j))) & " или " & str(round(procent*100)) & " %")
        ...
        if np = 0: 
            ...
            if tV33.count > 1:
                ...
                while ndx >= 0 # 
                    c_np.Z(ndx) = np_i
                    np_i = np_i + 1
                    logging.info("\t" &"\t" & "задан np ветви " & str(ip) & "," & str(iq) & "," & c_np.ZS(ndx) & "-" & tV33.cols.item("name").ZS(ndx))
...
class  uzel_mdp_class:   #  храним параметры узла и колекцию  генераторов 
    ...
    def init_rezerv_P (  ):                                                                                    #  РЕЗЕРВ конкретного узла
        ...
        if tip  = 1:                                # если нет генераторов в узле
            if pg_max > 0: rezerv_P_UP = pg_max - tN37.cols.item("pg").Z(ndxNode) else: logging.info( "в узле " & str(ny) & " " & name & "не задано поле pg_max" ) 
        ...
        if tip = 1 and KSC.tipIzm = 1:                           # если нет генераторов в УЗЛЕ и учитывем узлы без генераторов
            if up_pgen: # увеличиваем генерацию узла, KSC.KefPG_UP
                ...
                if pg_min < pgNode - deltaPG or pg_min = pgNode - deltaPG:  # (от pg_min (включительно) до pgNode)
                    ...
                    KSC.izm_ps = KSC.izm_ps - pgNode            kod = rastr.rgm(""): if kod!=0: kod = rastr.rgm(""): if kod!=0: kod = rastr.rgm(""): if kod!=0: kod = rastr.rgm("p"):  if kod!=0: kod = rastr.rgm("p")
            if kod != 0 OR fKontrSech (ns)  = False:    # fKontrSech возвращает истина если мощность в сечениях отмеченных контроль (sta) не превышена   или ложь (исключение)
                ...
                if kod!=0: logging.info ("\t" & "Аварийное завершение расчета режима, подвел узел ny = " & str(ny)): KSC.inini = KSC.Ncikl
                if kod!=0: exit def
            ...
            for EACH UGMC in DGen.Items
                ...
                if up_pgen and deltaPG != 0:          #  если УВЕЛИЧИТЬ ГЕНЕРАЦИЮ
                    if UGMC.Pmax >  0 and pgGen  <  UGMC.Pmax: #   если задан  UGMC.Pmax
                        ...
                        if UGMC.Pmax = 0: logging.info (  "\t" & "Не задан Pmax у генератора "& str(UGMC.Num) & "в узле " & str(ny) & "исключен из рассматрения" )
                ...
                if kod != 0 OR fKontrSech (ns)  = False:    # fKontrSech возвращает истина если мощность в сечениях отмеченных контроль (sta) не превышена   или ложь (исключение)
                    ...
                    kod = rastr.rgm(""): if kod!=0: kod = rastr.rgm(""): if kod!=0: kod = rastr.rgm(""): if kod!=0: kod = rastr.rgm("p"):  if kod!=0: kod = rastr.rgm("p")
                    if kod!=0: logging.info ("\t" & "Аварийное завершение расчета режима, подвел узел ny = "& str(ny) & "ген N= "& str(Num)): KSC.inini = KSC.Ncikl
                    if kod!=0: exit def
...
class  KorSech_class:   # KSC. изменить мощность сечения
    ...
    def korr (  ):  
        ...
        if ( db < dr_p_zad ):   logging.info ("Невозможно изменить мощность по сечению (с учетом всех узлов в модели)"):  exit def   
        ...
        if ( db < dr_p_zad ):   logging.info ("Невозможно изменить мощность по сечению (с учетом выбранных узлов)"):  exit def  
        ...
        for inini = 1 to Ncikl 
            ...
            if abs( izm_ps / newp) * 100 < epss  and izm_ps < newp and newp > 0: 
                logging.info ("\t" & "Заданная точность достигнута, итераций " & str( inini)& ",izm_ps="& str( round ( izm_ps))) 
                exit def #  выход  
            ...
            logging.info ("\t" & "\t" & "Мощность сечения = " & str(round (ps,0)) &" МВт, нужно получить: " &str (newp) &" МВт. Изменть на "  & str(round (izm_ps)) &" МВт. Отклонение = " & str(round (abs( izm_ps / newp) * 100)) & " %")
            logging.info ("\t" & "\t" &  "Количество узлов в выборке " & str(KolNySel) & ". Суммарный резерв на увеличении генерации " & str(round (rezerv_P_sum)) & " МВт.")
            logging.info ("\t" & "\t" &  "P снижение " & str(round (sum_gen_sech_DOWN))& " МВт. P увеличение " & str(round ( sum_gen_sech_UP ))& " МВт.")
            if KolNySel = 0:  logging.info ("\t" & "выход тк нет ген узлов"):   exit def
            if (sum_gen_sech_DOWN + sum_gen_sech_UP ) = 0:  logging.info ("\t" & "выход тк (sum_gen_sech_DOWN + sum_gen_sech_UP ) = 0"):   exit def
            calc_KefPG ()
...
def KorSech (ns,newp,vibor , tip , net_Pmin_zad ):  #  номер сеч, новая мощность в сеч (значение или "max" "min"), выбор корр узлов  (нр "sel"- отмеченные узлы и генераторыили "" - авто) ,  tip - pn или qn
    ...
    #  newp - инициализация
    if fNDX("sechen",ns) = -1:  logging.info ("\t" & "KorSech-выход тк нет сечения (возможно нужно загрузить файл сечений)" ): exit def       
    ...
    if tip = "pg":
        ...
        for i = 0 to 5
            ...
            if abs (P_izm) < 0.1: logging.info ("\t" & "заданная точность достигнута, сечение " & str( ns)& ",P_izm="& str( round ( P_izm))): exit for
            tN39.setsel(vibor)   
    ...
    while ndxSech >= 0
        if tS.cols.item("pmax").Z(ndxSech) AND tS.cols.item("ns").Z(ndxSech) != ns:
            if tS.cols.item("psech").Z(ndxSech)  > tS.cols.item("pmax").Z(ndxSech):  
                fKontrSech = False
                logging.info ("\t" & "превышение в сечении  " & tS.cols.item("ns").ZS(ndxSech))        ndxSech = tS.FindNextSel(ndxSech)
    wend
# End def return
# +++++++++++++++++++++++КОРРРРРРРРРРРРРРРРРРР +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
def fZ (tabl , kkluch , param):  #   возвращает заданное значение 
    ...
    if NDX == -1:
        logging.info ( "\t" & "fZ в таблице " & tabl &  " не найдено "  & str (kkluch)): exit  def
...
def PGen_cor ( sel_gen):  
    ...
    while ndxGen >= 0
        if tG.cols.item("Pmax").Z(ndxGen) < tG.cols.item("P").Z(ndxGen) and tG.cols.item("Pmax").Z(ndxGen) > 0:
            logging.info ("\t" & "изменена генерация генератора: " & tG.cols.item("Name").ZS(ndxGen) & ", номер: " & tG.cols.item("Num").ZS(ndxGen) & ", c " & tG.cols.item("P").ZS(ndxGen) & " на " & tG.cols.item("Pmax").ZS(ndxGen)& ", ny=" & tG.cols.item("Node").ZS(ndxGen))
            tG.cols.item("P").Z(ndxGen) = tG.cols.item("Pmax").Z(ndxGen)

        if tG.cols.item("Pmin").Z(ndxGen) > tG.cols.item("P").Z(ndxGen):
            logging.info ("\t" & "изменена генерация генератора: " & tG.cols.item("Name").ZS(ndxGen) & ", номер: " & tG.cols.item("Num").ZS(ndxGen) & ", c " & tG.cols.item("P").ZS(ndxGen) & " на " & tG.cols.item("Pmin").ZS(ndxGen)& ", ny=" & tG.cols.item("Node").ZS(ndxGen))
            tG.cols.item("P").Z(ndxGen) = tG.cols.item("Pmin").Z(ndxGen)

        if tG.cols.item("P").Z(ndxGen) = 0 and tG.cols.item("sta").Z(ndxGen) = 0:
            logging.info ("\t" & "генератор отключен: " & tG.cols.item("Name").ZS(ndxGen) & ", номер: " & tG.cols.item("Num").ZS(ndxGen) & ", P=" & tG.cols.item("P").ZS(ndxGen) & ", ny=" & tG.cols.item("Node").ZS(ndxGen))
            tG.cols.item("P").Z(ndxGen) = tG.cols.item("Pmin").Z(ndxGen)
...
class import_class:  #  класс импорта данных из файлов rg2
    ...
    def export_csv (): #  экспорт из файла
        ...
        logging.info ("\t" & "import_class.export_csv: файл:" &  import_File)

        for ii = 0 to ubound (tabl_zad)
            logging.info ("\t" & "экспорт таблица: "& tabl_zad (ii) )
            logging.info ("\t" & "выборка: " & vibor_zad (ii) )
            logging.info ("\t" & "параметры: " & param_str (ii))
            ExportCSV (  file_csv (ii), tabl_zad (ii) , param_str (ii) , vibor_zad (ii))

    def import_csv (): #  импорт в файлы
        if RG.test_name (uslovie_start):  # ИМПРТ в КОР Файл
            logging.info ("\t" & "import_class.import_csv: файл:" &  import_File)
            for ii = 0 to ubound (tabl_zad)
                logging.info ("\t" & "импорт таблица: "&  tabl_zad (ii) )
                logging.info ("\t" &"выборка: " & vibor_zad (ii) )
                logging.info ("\t" & "тип: " & tip_zad (ii) )
                logging.info ("\t"  & "параметры: " & param_str (ii) )
                logging.info ("\t" & "файл импорта: "&  file_csv (ii))
...
class print_korr_class:  #  класс печать данных в excel
    ...
    def init_pkc (): #   добавить листы и ссылки на них
        ...
        boor_print = GS.excel.Workbooks.Add      
        if GLK.print_sech: 
            if rastr.Tables.Find("sechen") < 0:
                logging.info( "!!! НЕ ЗАГРУЖЕН ШАБЛОН сечений с таблицей  sechen (.sch) !!" )
                GLK.print_sech = False
            else:
                if rastr.Tables("sechen").size = 0:  
                    logging.info( "!!! НЕ ЗАГРУЖЕН ФАЙЛ СЕЧЕНИЯ (.sch) !!" )
                    GLK.print_sech = False
...
:XL_print_param_log (XL_print_darea_log  ,  XL_print_darea_p_log & ",d")  
    ...
    def XL_print_name_RGM (print_yyy , LIST):  
        ...
        LIST.Cells(print_yyy,3).Value = RG.name_list (1)
        LIST.Cells(print_yyy,4).Value = RG.name_list (2)
        ...
        while ndx_tG >= 0          # 
            if not tG.cols.item("sta").Z(ndx_tG) = 1: 
                if zad = "min":
                    ...
                    f_gen_qg = f_gen_qg + tG.cols.item("Qmax").Z(ndx_tG)            ndx_tG = tG.FindNextSel(ndx_tG)
        wend   
    # End def return
    ...
    def finish (): #  
        ...
        if GLK.printXL and GLK.print_save: 
            if GLK.korr_papka_file = 1: 
                if GLK.KInFolder != "": boor_print.SaveAs (GLK.KInFolder  & "\протокол коррекция файлов (" & str (Day(Now)) & "_" & str (Month(Now)) & "_" & str (Year(Now)) & "г " & str (Hour(Now)) & "ч_" & str (Minute(Now)) & "м_" & str (Second(Now)) & "c" & ").xlsm" , 52)
                if GLK.KInFolder = "": boor_print.SaveAs (GLK.KIzFolder & "\протокол коррекция файлов (" & str (Day(Now)) & "_" & str (Month(Now)) & "_" & str (Year(Now)) & "г " & str (Hour(Now)) & "ч_" & str (Minute(Now)) & "м_" & str (Second(Now)) & "c" & ").xlsm" , 52)
    ...
    def Svod_kor( tabl ):      #  сводные (ссылка на объект таблица)
        ...
        if Not (tabl.Name = "Таблица_na" Or tabl.Name = "Таблица_npa" Or tabl.Name = "Таблица_no" Or tabl.Name = "Таблица_sech" Or tabl.Name = "Таблица_tab"): logging.info( tabl.Name + "- не та таблица"): Exit def
...
def kor1  (k_kluch , param_kor , value_param): #  коррекция одного уникальнгого занчения(краткийй ключ, параметр корр, значение) например("7","name","Юж")
    ...
    if ubound (kkluch_m) = 0: #  УЗЕЛ
        ...
        if kkluch_m (0) = "npa": #  территория
            ...
            viborka = "npa=" & str (kkluch_m(1))    ptabl.setsel(viborka)
...
def cor_one  (tabl,param,viborka,formula): #  коррекция одного уникальнгого занчения(таблица, параметр корр, выборка, формула для расчета параметра) "node","name","ny=7","Юж" УСТАРЕЛО
    ...
    if ndx > -1: 
        ...
        logging.info ("\t" & "не найден: " &  str(tabl) & " / " & str(param) & " / " & str(viborka) & " / " & str(formula))

    logging.info ("\t" & "cor_one: tabl=" & tabl & "/param=" & param & tabl & "/viborka=" & viborka & tabl & "/formula=" & formula )
    ...
    if viborka = "net":
        if tabl = "node":
            ...
            while it >= 0 
                ...
                if tVet.count = 0:
                    logging.info ("\t" & "\t" & "узел без связей Del: tabl=" & tabl & "/viborka=" & "ny=" & ptabl.cols.item("ny").ZS(it)&"/"&ptabl.cols.item("name").ZS(it) )
            ...
            while it >= 0 
                ...
                if tNod.count < 2:
                    logging.info ("\t" & "\t" & "ветв без узла начали конча Del: tabl=" & tabl & "/viborka=" & ptabl.SelString(it) &"/" & ptabl.cols.item("name").ZS(it) )
...
def name0 ():            #  поиск узлов и генераторов без имени
    ...
    for i=0 to nodee.size-1
        if Replace(nodee.cols.item("name").ZS(i) , " ", "") = "": logging.info ("\t" & "узел без имени ny: " &  nodee.cols.item("ny").ZS(i))

    genn = rastr.tables("Generator")
    for i=0 to genn.size-1
        if Replace(genn.cols.item("Name").ZS(i) , " ", "") = "": logging.info ("\t" & "генератор без имени Num: " &  genn.cols.item("Num").ZS(i))
...
def vzd0 ():            #  поиск узлов где напряжение vzd задано а диапозона реактивки нет и удаляет vzd
    ...
    for i=0 to nodee.size-1
        if nodee.cols.item("vzd").Z(i) > 0 and nodee.cols.item("qmin").Z(i) = 0 and nodee.cols.item("qmax").Z(i) = 0: 
            logging.info ("\t" & "узел c qmin=qmax=0 vzd = " &  nodee.cols.item("vzd").ZS(i) & " ny = " &  nodee.cols.item("ny").ZS(i) & "("& nodee.cols.item("name").ZS(i)&  "), vzd изменено на 0")
...
def set_tab (kluch):  #  по ключу возвращает таблицу
    ...
    if isempty (set_tab): logging.info ("set_tab, не найден: " & kluch): msgbox "ОШИБКА, вывод заданных параметров, не найден: " & kluch
# End def return
# +++++++++++++++++++++++ОБЩЕЕ ДЛЯ МАКРОСОВ хранить в  коррррр++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
